...
var resolveOpenAIClient = (options) => {
  ...
  return azureOpenAi ? new AzureOpenAI({
...
// js/templates.ts
var modelGradedSpecSchema = z.object({
  prompt: z.string(),
...
function renderMessages(messages, renderArgs) {
  return messages.map((m) => ({
    ...m,
    content: m.content ? mustache.render(m.content, renderArgs, void 0, {
...
var ContextEntityRecall = makePartial(async (args) => {
  ...
  const makeArgs = (text) => ({
    ...
    messages: [
      {
        role: "user",
        content: mustache2.render(ENTITY_PROMPT, { text })
      }
    ],
    tools: [
      {
...
context: {{context}}
candidate sentences: `;
var relevantSentencesSchema = z2.object({
  sentences: z2.array(
    z2.object({
      sentence: z2.string().describe("The selected sentence"),
      reasons: z2.array(z2.string()).describe(
...
var ContextRelevancy = makePartial(async (args) => {
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "user",
        content: mustache2.render(SENTENCE_PROMPT, {
          question: input,
          context
        })
      }
...
classification:
`;
var contextRecallSchema = z2.object({
  statements: z2.array(
    z2.object({
      statement: z2.string(),
      attributed: z2.number(),
      reason: z2.string()
    })
  )
});
  ...
  async (args) => {
    ...
    const response = await client.chat.completions.create({
      ...
      messages: [
        {
          role: "user",
          content: mustache2.render(CONTEXT_RECALL_PROMPT, {
            question: input,
            answer: expected,
            context
          })
...
verification:
`;
var contextPrecisionSchema = z2.object({
  reason: z2.string().describe("Reason for verification"),
  verdict: z2.number().describe("Binary (0/1) verdict of verification")
});
var ContextPrecision = makePartial(async (args) => {
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "user",
        content: mustache2.render(CONTEXT_PRECISION_PROMPT, {
          question: input,
          answer: expected,
          context
        })
...
answer:
`;
var extractedStatementsSchema = z2.object({
  statements: z2.array(z2.string()).describe("the list of extracted statements")
});
var statementFaithfulnessSchema = z2.object({
  faithfulness: z2.array(
    z2.object({
      statement: z2.string().describe("the original statement, word-by-word"),
      verdict: z2.number().describe("the verdict(0/1) of the faithfulness."),
      reason: z2.string().describe("the reason of the verdict")
    })
  )
});
  ...
  async (args) => {
    ...
    const extractedStatementsResponse = await client.chat.completions.create({
      ...
      messages: [
        {
          role: "user",
          content: mustache2.render(LONG_FORM_ANSWER_PROMPT, {
            question: input,
            answer: output
          })
        }
    ...
    const faithfulnessResponse = await client.chat.completions.create({
      ...
      messages: [
        {
          role: "user",
          content: mustache2.render(NLI_STATEMENTS_PROMPT, {
            context,
            statements
          })
        }
...
output:
`;
var questionGenSchema = z2.object({
  question: z2.string(),
  noncommittal: z2.number()
});
    ...
    Array.from(
      ...
      () => client.chat.completions.create({
        ...
        messages: [
          {
            role: "user",
            content: mustache2.render(QUESTION_GEN_PROMPT, {
              answer: output,
              context
            })
          }
...
`;
var answerCorrectnessClassificationSchema = z2.object({
  TP: z2.array(z2.string()),
  FP: z2.array(z2.string()),
  FN: z2.array(z2.string())
});
  ...
  const [factualityResponse, answerSimilarityResult] = await Promise.all([
    client.chat.completions.create({
      ...
      messages: [
        {
          role: "user",
          content: mustache2.render(CORRECTNESS_PROMPT, {
            question: input,
            answer: output,
